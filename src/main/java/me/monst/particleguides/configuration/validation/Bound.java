package me.monst.particleguides.configuration.validation;
...
/**
 * A {@link Bound} is a {@link Predicate} that checks whether a value is within a certain range. If it is not, it
 * replaces the value with a different one by throwing a {@link ValueOutOfBoundsException}.
 *
...
public interface Bound<T> extends Bounds<T> {
    ...
    /**
     * Returns a bound which requires the {@link Comparable} value to be at least {@code min}. If not, the value is
    ...
    /**
     * Returns a bound which requires the {@link Comparable} value to be at most {@code max}. If not, the value is
     * replaced with {@code max}.
     *
    ...
    /**
     * Returns a bound which requires the value to fulfill the provided predicate, mapping it to a replacement value if
     * it does not. In a typical use case, the mapper produces values which consistently fulfill the predicate.
     *
    ...
    /**
     * Returns a bound which forbids the value from fulfilling the provided predicate, mapping it to a replacement value
     * if it does. In a typical use case, the mapper produces values which consistently do not fulfill the predicate.
     *
